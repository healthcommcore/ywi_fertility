...
sourcerer_init = function()
{
	...
	button = document.id( 'sourcetags_button' );
	if ( string.search( '{'+sourcerer_syntax_word ) != -1  ) {
		button.removeClass( 'sourcetags_0' ).addClass( 'sourcetags_1' );
	} else {
		button.removeClass( 'sourcetags_1' ).addClass( 'sourcetags_0' );
	...
	button = document.id( 'tagstyle_button' );
	if ( string.search( /\[\[/g ) != -1 && string.search( /\]\]/g ) != -1  ) {
		button.removeClass( 'tagstyle_0' ).addClass( 'tagstyle_1' );
	} else {
		button.removeClass( 'tagstyle_1' ).addClass( 'tagstyle_0' );
	}
};
...
sourcerer_insertText = function()
{
	// define variables used in function
	var spans, regex;
	var keepcolors = document.id( 'keepcolors' );
	var keepindent = document.id( 'keepindent' );
...
sourcerer_toggleSourceTags = function()
{
	// define variables used in function
	var regex;
	var button = document.id( 'sourcetags_button' );
	var string = editAreaLoader.getValue( 'source' );

	if ( button.hasClass( 'sourcetags_0' ) ) {
...
{
	// define variables used in function
	var button = document.id( 'tagstyle_button' );
	var string = editAreaLoader.getValue( 'source' );

	if ( button.hasClass( 'tagstyle_0' ) ) {
...
sourcerer_loadSyntax = function( syntax, is_waiting, colorize )
{
	...
	if ( !editAreaLoader.load_syntax[syntax] ) {
		...
		frame_source = document.id( 'frame_source' );
